...
print(np.mean(np.abs(np.asarray(fc)-np.asarray(c))))
print(np.mean(np.abs(np.asarray(ffc)-np.asarray(cc))))
...
# MFEAT NOISY
c, c2, a, a2, l, l2, fm, fm2, fr, fr2, fc, fc2 = get_results("data/mfeat/results_sans_weights_noisy/")
cc, cc2, aa, aa2, ll, ll2, ffm, ffm2, ffr, ffr2, ffc, ffc2 = get_results("data/mfeat/results_standard/")
plot(fm, ffm, "MSE", "mse_noisy_mfeat")
plot(fr, ffr, "MRD", "mrd_noisy_mfeat")
plot(fc, ffc, "CS", "cs_noisy_mfeat")
plot(np.asarray(fc)-np.asarray(c), np.asarray(ffc)-np.asarray(cc), "CD", "cd_noisy_mfeat")
print(np.mean(np.abs(np.asarray(fc)-np.asarray(c))))
print(np.mean(np.abs(np.asarray(ffc)-np.asarray(cc))))